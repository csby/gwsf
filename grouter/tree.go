...
type node struct {
	...
	priority  uint32
	children  []*node
	handle    gtype.HttpHandle
	before    gtype.HttpHandle
}
...
func (n *node) addRoute(path string, handle, before gtype.HttpHandle) {
	fullPath := path
	n.priority++

	// Empty tree
	if len(n.path) == 0 && len(n.indices) == 0 {
		n.insertChild(path, fullPath, handle, before)
		n.nType = root
		return
	}
...
func (n *node) insertChild(path, fullPath string, handle, before gtype.HttpHandle) {
	for {
...
// given path.
func (n *node) getValue(path string, params func() *gtype.Params) (handle, before gtype.HttpHandle, ps *gtype.Params, tsr bool) {
